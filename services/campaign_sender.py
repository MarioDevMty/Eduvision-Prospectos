...
def _send_recipient(
    ...
    campus = (recipient["campus_name_snapshot"] or "").strip()
    ...
    try:
        ...
        error = f"No fue posible preparar el mensaje: {exc}"
        update_recipient_status(
            recipient_id=recipient_id,
            status="ERROR",
            user_id=user_id,
    ...
    result = send_campaign_email(
        ...
        smtp_security=smtp_config.get("security", "starttls"),
        recipient=email,
        subject=campaign["subject"],
        body_text=campaign["body_text"],
        attachment_paths=[],
    ...
    if result["success"]:
        mark_email_send_attempt_accepted(
            ...
            sent_folder_saved=result["sent_folder_saved"],
            sent_folder_name=result["sent_folder_name"],
        )
        update_recipient_status(
            recipient_id=recipient_id,
            status="ENVIADO",
        ...
        update_recipient_status(
            ...
            status="ERROR",
            user_id=user_id,
            details=f"{result['message']} Message-ID: {message_id}",
        )
        status = "ERROR"
...
def send_single_campaign_recipient(
    ...
    if recipient is None:
        return {"success": False, "message": "El destinatario no existe."}
    if int(recipient["campaign_id"]) != int(campaign_id):
        return {
            "success": False,
            "message": "El destinatario no pertenece a esta campaña.",
        }
    if recipient["status"] != "PENDIENTE":
        return {
            "success": False,
            "message": "Solo se envían registros PENDIENTES.",
        }
...
def send_next_campaign_batch(
    ...
    selected = [r for r in recipients if r["status"] == "PENDIENTE"][:batch_size]

    if not selected:
        return {
            "success": True,
            "message": "No existen destinatarios pendientes.",
            "processed": 0,
            "sent": 0,
            "errors": 0,
            "remaining": 0,
    ...
    for index, recipient in enumerate(selected):
        ...
        sent_copies += 1 if result.get("sent_folder_saved") else 0
        results.append({
            "plantel": result.get("plantel", ""),
            "email": result.get("email", ""),
            "status": result.get("status", "ERROR"),
            "message_id": result.get("message_id", ""),
            "copia_en_enviados": "Sí" if result.get("sent_folder_saved") else "No",
            "message": result["message"],
        })

        if index < len(selected) - 1:
            time.sleep(max(int(interval_seconds), 0))

    remaining = sum(
        1
        for row in get_campaign_recipients(campaign_id)
